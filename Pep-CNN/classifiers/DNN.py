...
data_=pd.read_csv(r'F:/lxj/研究/code/mycode-PPTPP/tests feature/feature_AIP_test.csv', header = None)
data=np.array(data_)
data=data[:,1:]
[m1,n1]=np.shape(data)
#label1=np.ones((int(m1/2),1))#Value can be changed
#label2=np.zeros((int(m1/2),1))
label1 = np.ones((420,1))
label2 = np.zeros((629,1))
...
#yscore_sum.to_csv('yscore_DNN_AIP.csv')
ytest=ytest[np.array(range(1,row)),:]
ytest_sum = pd.DataFrame(data=ytest)
#ytest_sum.to_csv('ytest_DNN_AIP.csv')
...
data_csv.to_csv('DNN_AIP_test.csv')
